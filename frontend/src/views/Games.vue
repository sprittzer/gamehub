  ...
  <div class="games-page">
    <div class="games-header">
      ...
      <div class="filters-row compact-filters">
        ...
        <div class="filter-item slider-wrapper">
          <label class="slider-label">Рейтинг: {{ ratingRange[0].toFixed(1) }} - {{ ratingRange[1].toFixed(1) }}</label>
          <Slider
            v-model="ratingRange"
            range
            :min="0"
        ...
        <div class="filter-item buttons-group">
          ...
          <Button label="Сбросить" icon="pi pi-refresh" outlined class="ml-2" @click="resetFilters"/>
        </div>
      </div>
    </div>
...
const applyFilters = () => {
  ...
  filters.value.platforms = selectedPlatforms.value.length ? selectedPlatforms.value.join(',') : null
...
// --- Сброс фильтров ---
const resetFilters = () => {
  filters.value = { q:'', genres:null, platforms:null, developer:'', min_year:null, max_year:null, min_rating:null, max_rating:null }
...
.compact-filters .filter-item { min-width: 150px; flex: 1 1 auto; }
.slider-wrapper { flex: 1 1 200px; display: flex; flex-direction: column; }
.slider-label { font-size: 0.85rem; margin-bottom: 0.2rem; color: #ccc; }
.developer-input { flex: 1 1 150px; }
.buttons-group { display: flex; gap: 0.5rem; flex-shrink: 0; }
