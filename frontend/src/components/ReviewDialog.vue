<template>
  <Dialog :visible="visible" modal :style="{ width:'500px' }" @update:visible="$emit('update:visible', $event)">
    <template #header>
...
const saveReview = async () => {
  ...
  if(localReview.value.id) {
    res = await fetch(api.reviews.update(localReview.value.id), {
      method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify(localReview.value)
    })
  } else {
    res = await fetch(api.reviews.create(), {
      method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(localReview.value)
    })
  }
